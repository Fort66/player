...
from PySide6.QtWidgets import QMainWindow, QFileDialog, QMenu, QSystemTrayIcon
...
class MenuWindow(QMainWindow, Ui_MainWindow):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.tray.setIcon(QIcon(':/images/videos_video_media_tray.png'))
        self.tray.setVisible(True)

        # меню в системном трее
        self.menu = QMenu(self)

        self.add_submenu = QMenu('Добавить файлы', self)
        self.add_submenu.setIcon(QIcon(':/images/icons/odata.png'))

        self.action_add_audio_files = self.add_submenu.addAction('Аудио', self.add_audio_files)
        self.action_add_video_files = self.add_submenu.addAction('Видео', self.add_video_file)

        self.menu.addMenu(self.add_submenu)

        self.menu.addSeparator()

        self.action_prev = self.menu.addAction('Предыдущий трек', self.previos_file)
        self.action_prev.setIcon(QIcon(':/images/icons/control-skip-180.png'))
        self.action_next = self.menu.addAction('Следующий трек', self.next_file)
        self.action_next.setIcon(QIcon(':/images/icons/control-skip.png'))

        self.menu.addSeparator()

        self.action_pause_file = self.menu.addAction('Пауза', self.pause_file)
        self.action_pause_file.setIcon(QIcon(':/images/icons/control-pause.png'))
        self.action_play_file = self.menu.addAction('Воспроизвести', self.play_file)
        self.action_play_file.setIcon(QIcon(':/images/icons/control.png'))

        self.menu.addSeparator()
        self.menu.addAction('Выход', self.exit_app)
