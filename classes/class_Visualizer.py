...
class AudioAnalysisThread(QThread):
    ...
    def run(self):
        ...
        brigh_previos = [0, 0, 0]  # Список для хранения прошлых значений яркости
        scale_previos = [1.0, 1.0, 1.0]  # Список для хранения прошлых значений масштаба
        history = [[0.0] * HISTORY_FRAMES for _ in range(3)]  # История громкости для 3 полос
        ...
        while self.is_running:  # Пока флаг работы активен
            ...
            for i in range(3):  # Цикл по НЧ, СЧ, ВЧ
                start, end = fft_cfg[i]['rng']  # Извлекаем границы диапазона спектра

                val = np.mean(fft[start:end]) if end < len(fft) else 0  # Средняя громкость полосы

                avg = sum(history[i]) / HISTORY_FRAMES  # Вычисляем средний фон за историю
                ...
                history[i].pop(0)  # Удаляем старое значение из истории
                history[i].append(val)  # Добавляем новое значение в историю
            ...
            for i in range(3):  # Цикл сглаживания данных
                ...
                rate = SMOOTHING_RATE_RISE if rgb_now[i] > brigh_previos[i] else SMOOTHING_RATE_FALL

                brigh_previos[i] = int(brigh_previos[i] * (1 - rate) + rgb_now[i] * rate)  # Формула сглаживания

                out_rgb[i] = max(0, min(255, brigh_previos[i]))  # Ограничиваем цвет
                scale_factor = 1.0 + min(SCALE_PULSE_MAX, out_rgb[i] / SCALE_PULSE_THRESHOLD)  # Рассчитываем пульсацию

                scale_previos[i] = scale_previos[i] * SMOOTHING_RATE_FALL + scale_factor * (1 - SMOOTHING_RATE_FALL)

                out_scale[i] = scale_previos[i]  # Записываем результат масштаба
